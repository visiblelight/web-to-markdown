...
典型场景：用 Claude Code 或者 OpenClaw 抓取一篇微信公众号文章，生成的 Markdown 需要发送给其他机器或服务使用。如果图片保存在本地，其他机器无法访问；如果直接引用原始平台的源图片，也可能会被屏蔽（公众号就是这样）。因此需要一个对象存储服务来托管图片，Markdown 中使用公网 URL。
